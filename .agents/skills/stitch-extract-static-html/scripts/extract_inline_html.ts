  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      const fallback =
        "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";
      console.warn(
        `  WARN: Invalid URL: ${url.slice(0, 70).replace(/\n|\r/g, "")}...`,
      );
      imgCache.set(url, fallback);
      resolve(fallback);
      return;
  ...
  traverse(ast, {
    ExportDefaultDeclaration(path) {
      ...
      if (decl.type === "ArrowFunctionExpression") {
        if (
          decl.body.type === "JSXElement" ||
          decl.body.type === "JSXFragment"
        ) {
...
function readCssFile(filePath: string | null): {
  ...
  if (!filePath || !fs.existsSync(filePath))
    return { imports: [], css: "", hasApply: false };
...
function extractFromHtml(htmlPath: string | null): {
  styles: string;
  links: string[];
} {
...
function buildHead(opts: Opts): string {
  ...
  if (useTailwind && !tailwindConfig) {
    ...
    if (tailwindConfig)
      console.log(`Auto-detected Tailwind config: ${tailwindConfig}`);
    else {
      useTailwind = false;
  ...
  // Build head
  const htmlAttrs = opts.htmlClass
    ? ` lang="en" class="${opts.htmlClass}"`
    : ' lang="en"';
